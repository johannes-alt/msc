...
In each of these holes, a PMMA vessel is placed to house the two \acp{wom}.
...
\begin{figure}
	\centering
	\includegraphics[width=.5\textwidth]{pictures/one_cell}
	\caption[One Cell Prototype]{The One Cell Prototype at the DESY testbeam area.}% a) and a PMMA vessel with a Wavelenth-shifting Optical Module inserted and a \ac{sipm} board attached b). \cite{}}
	\label{fig:one_cell}
\end{figure}

The container is filled with \ac{lab} mixed with \SI{2}{\gram\per\liter} Diphenyloxazole (PPO)%, which is also planed to be used in the \ac{sbt}.
...
Most of the scintillation light has a wavelength of \SIrange{340}{420}{\nano\meter}.
...
\begin{figure}
	...
	\caption[LAB emission spectrum]{The emission spectrum of the liquid scintillator \ac{lab} with \ac{ppo} and the wavelength shifter Bis-MSB. \cite{Schmidt}}
...
For a small number of photons hitting the \ac{sipm} simultaneously, the probability of multiple photons hitting the same pixel is low.
Therefore, then one can count photons \cite{ACERBI201916}.
...
\end{figure}
It has eight input channels, each equipped with a $\approx\SI{1}{\volt}$ anode voltage control to equalize the overvoltage between the different channels.
...
The \ac{emusic} has a low attenuation setting, with which the signal attenuation through the pole-zero shaper can be reduced.
...
\begin{figure}
	...
	\caption[eMUSIC pole-zero cancellation]{Sketch of a pole-zero cancellation with resistors adjustable in 8 steps and capacitor adjustable in 32 steps, the input voltage $\text{V}_\text{i}$, the output voltage $\text{V}_\text{o}$, and the operation voltage $\text{V}_\text{cc}=\SI{3.3}{\volt}$. \cite{electronics10080961}}
...
The unit \si{\ohm} of the gain is due to the \ac{emusic} being a transimpedance amplifier with a current input and voltage output.
The response of the \ac{emusic} is linear only in its first half of the dynamic range.
...
A fotograph of the board is shown in \autoref{fig:emusic_board}.
...
The connectors on the \ac{emusic} board and the corresponding function are listed in \autoref{tab:emusic_board_connectors}
...
\begin{table}
	\centering
	\caption[eMUSIC board connectors]{The connectors on the \ac{emusic} board with their corresponding function.}
	\label{tab:emusic_board_connectors}
	\begin{tabularx}{\textwidth}{X|X}\toprule
		connector & function \\\midrule
		P1 & supplying power to the six LEDs \\
		P2 & HV supply for the \acp{sipm} \\
		P3 & configuring the \ac{emusic} \\
		K1 - K8 & signal output for channel 0 to 7 \\
		K9 & Fast OR output \\ 
		J1 & connector for the \ac{sipm} board \\ 
		J3 & supplying power to the \ac{emusic} board \\ 
		J4 & programming the ATmega328P-AU \\
		J6 & differential summation signal output \\
		\bottomrule
	\end{tabularx}
\end{table}
...
\subsection{Usage of the \ac{gandalf} with \acp{sipm}}
In this work, the \ac{gandalf} was used for digitizing the output signal of the \ac{emusic} \ac{asic}.
The output signals of the \ac{emusic} have a positive signal polarity.
